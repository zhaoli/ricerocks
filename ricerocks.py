...
def rock_spawner():
    ...
    if (dist(rock_pos, my_ship.get_position()) + 5) <= (my_ship.get_radius() + 40):
        return
    rock_vel = [random.random() * .6*(1 + 0.01 * score) - .3, random.random() * .6*(1 + 0.01 * score) - .3]
